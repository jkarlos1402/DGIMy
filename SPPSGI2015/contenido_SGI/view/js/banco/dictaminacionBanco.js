...
$(document).ready(function () {
    ...
    $("#mostraredit").click(function () {
        if ($("#tIdBco").text() !== "") {
            bootbox.alert('Se debe limpiar la pantalla antes de analizar un nuevo estudio socioecon\u00f3mico', function () {
                location.reload();
            });
        } else {
            if ($("#numerobanco").val().length == 0) {
                bootbox.alert('Ingresa un No. de Banco a buscar');
                return false;
            } else {
                obtenerInfoBco();
            }
    ...
    $("#CmbTipEva").change(function () {
        cambioTipoEva();
    });



    $("#btnEvaluacionPdf").click(function () {

        $("#formPdf").html("<form method='POST' action='contenido_SGI/view/Banco/dictamenPDF.php' target='_blank'>" +
                "<input type='text' name='idBco' value='" + $("#tIdBco").html() + "' />" +
                "<input type='text' name='idSol' value='" + $("#tIdSol").html() + "' />" +
                "<input type='text' name='idEva' value='" + $("#valEva").html() + "' />" +
                "<input type='submit' name='enviar' value='enviar' id='enviarPDF' />" +
                "</form>");
        $("#enviarPDF").click();
    });

    $("#tfTasDes").autoNumeric({aSep: '', mDec: 2, vMin: '0.00', vMax: "100.00"});
    $("#tfVAN,#tfVACPta,#tfCAEPta,#tfVACAlt,#tfCAEAlt").autoNumeric({aSep: ',', mDec: 2, vMin: '0.00'});
    $("#tfTIR,#tfTRI").autoNumeric({aSep: '', mDec: 2, vMin: '0.00', vMax: '999.99'});
    $("#numerobanco").autoNumeric({aSep: '', mDec: 0});
    $("#tFecIng").hide();
});

function cambioTipoEva(callback) {
            ...
            success: function (response) {
                ...
                switch (tipoEvaluacionTmp) {
                    case "1"://ficha tecnica
                        if (parseFloat($("#tMonto").val().replace(/,/g, "")) >= 30000000 && parseFloat($("#tMonto").val().replace(/,/g, "")) <= 50000000) {
                            $("#row_36").show();
                            $("#row_37").show();
                            $("#row_38").show();
                            $("#row_39").show();
            ...
            success: function (response) {
                ...
                if (info.ingresoFisico) {
                    ...
                    sendNotification("", $("#idUsuarioSession").val(),
                            "", "", $("#idRolUsuarioSession").val(), generales.IdBco, "4", "");// notificacion
                ...
                if (info.observaciones) {
                    if (info.AccionSeguir !== "1") {
                        ...
                        sendNotification("", $("#idUsuarioSession").val(),
                                "", "", $("#idRolUsuarioSession").val(), generales.IdBco, "2", "");// notificacion
