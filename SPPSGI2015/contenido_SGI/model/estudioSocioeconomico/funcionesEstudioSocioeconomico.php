...
include_once 'funcionesEstudioSocioeconomicoModel.php';
...
//    print_r($datos);    
        foreach ($datos as $value) {
            array_push($_SESSION["ex"], $value);
        }
//        ChromePhp::log($_SESSION['ex']);
        if (count($_SESSION["ex"] > 0))
            return true;
        else
            return false;
    }
    ...
    function buscaObra($id) {
        $idSol = $this->instanciaExpTecModel->buscarObra($id);
        if (count($idSol) > 0) {
            $idue = $_SESSION["IDUE"];
            $solicitud = $this->instanciaExpTecModel->buscarSolicitud($idSol, $idue);
            if (count($solicitud) > 0) {
    ...
    function buscaSol($id) {
        $idSol = $this->instanciaExpTecModel->buscarObra($id);
        if (count($idSol) > 0) {
            $idue = $_SESSION["IDUE"];
            $solicitud = $this->instanciaExpTecModel->buscarSol($idSol, $idue);
            if (count($solicitud) > 0) {
                $accSolicitud = $this->instanciaExpTecModel->buscarAccionesFed($idSol);
                $accSolicitudEst = $this->instanciaExpTecModel->buscarAccionesEst($idSol);
                $acuSolicitud = $this->instanciaExpTecModel->buscarAcuSolicitud($idSol);
    ...
    public function guardarMontos($idSol, $estado, $tiposolicitud, $montoautorizado) {
        if ($estado == 6) {
            switch ($tiposolicitud) {
                case 2:
                case 12:
                    $montos = $this->instanciaExpTecModel->guardarMontoAutorizado($idSol, $montoautorizado);
                    break;
                case 3:
                case 9:
                    $montos = $this->instanciaExpTecModel->guardarMontoAsignado($idSol, $montoautorizado);
                    break;
                case 10:
                case 11:
                case 13:
                    $montos = $this->instanciaExpTecModel->guardarMontos($idSol, $montoautorizado);
                    break;
                case 4:
                    $montos = $this->instanciaExpTecModel->guardarMontosReduccion($idSol);
                    break;
            }
            if (count($montos) > 0) {
                return $montos;
    ...
    public function consultasObras($fecIni, $fecFin) {
        ...
        $oficios = $this->instanciaExpTecModel->consultarObrasOficios($idusu, $fecIni, $fecFin);
        $data = array('obra' => $obra, 'oficios' => $oficios);
        return $data;
    }
    ...
    public function enviarDictaminar($idBco){
        return $this->instanciaExpTecModel->enviarDictaminar($idBco);
    }
}